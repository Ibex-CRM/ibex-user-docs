...
Campaign Search lives in the Ibex 2.0 Dashboard. After logging in with your Okta credentials, you'll find it waiting for you at the bottom of the dashboard page - like finding the last piece of chocolate in the box! 🍫
...
This tool is your comprehensive campaign archive, building on the foundation of the original Ibex platform with enhanced features and improved navigation. Think of it as your personal campaign librarian! 📚
...
- 👋 Have we worked with [creator handle or name]?

<Callout type="info" emoji="🔍">
  **Search Tip:** Looking to see if we've worked with a specific creator? Just type their handle or name in the search bar! 💪
</Callout>
...
The search bar is your precision tool for finding specific campaigns 🔍  
Use it to:
...
<Callout type="info">
  **Search Strategy:** Use the search bar for specific identifiers and the filtering system for broader categories - it's like having both a microscope and a telescope! 🔬🔭
</Callout>

### 🎛️ The Power of Filters

1. Search for non-numerical data (e.g., brand, campaign name, territory, objective, vertical)
2. Search for specific creators using their:
   - Channel handle
   - Display name (e.g., Wendy Barker)
   - Email address

The search bar works best for finding specific items (campaign names, codes, brands, or unique identifiers). For broader categories (beauty, brand awareness), the filtering system is more effective as some terms appear in multiple filters (e.g., beauty appears in both verticals and product categories).

#### Filtering and sorting

Most table columns can be sorted using the chevron icon next to the column header. By default, the table sorts by date (newest first).

Access the filters by clicking the filter button to the right of the search bar. There are 11 filter groups, each containing 1-2 filters and an AND/OR toggle that can be used to tune the specificity of the results.
